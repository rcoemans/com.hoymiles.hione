...
class HoymilesHybrid {
  ...
  /**
   * Query cloud for gateway device online status.
   * Returns true if the cloud device listing shows the gateway as online.
   * Falls back to local connectivity check if cloud is unavailable.
   * @returns {Promise<boolean>}
   */
  async getGatewayStatus() {
    // Try cloud device listing for gateway status
    if (this._email && this._password && this._stationId) {
      try {
        await this._cloud.ensureToken(this._email, this._password);
        const devices = await this._cloud.getDevices(this._stationId);
        if (devices && devices.gateway) {
          const raw = devices.gateway.raw || {};
          // Check multiple possible status field names
          const status = raw.status ?? raw.device_status ?? raw.online ?? raw.network_status;
          const statusStr = String(status ?? '').toLowerCase();
          const isOnline = statusStr === 'online' || statusStr === 'normal' || status === 1 || status === true;
          this.log(`[Hybrid] Gateway status from cloud: ${statusStr} → ${isOnline ? 'online' : 'offline'}`);
          return isOnline;
        }
      } catch (err) {
        this.log('[Hybrid] Cloud gateway status check failed: ' + err.message);
      }
    }
    // Fallback: use local connectivity
    if (this._local || this._modbus) {
  ...
  async _getLocalData() {
    ...
    if (this._modbus && this._modbusAvailable) {
      ...
      // Energy top-up from cloud when Modbus has no confirmed energy
      if (modbusData.confidence && modbusData.confidence.totalEnergy === 'none' && this._cloudFallbackEnabled) {
        await this._topUpModbusFromCloud(modbusData);
      }
      return modbusData;
    }
  ...
  /**
   * Generate a comprehensive Modbus scan report for discovery/analysis.
   * Combines known blocks scan + ESS probe into a shareable report.
   * @returns {Promise<object|null>}
   */
  async exportModbusScanReport() {
    if (!this._modbus) return null;
    return this._modbus.exportScanReport();
  }

  /**
   * Fetch an API snapshot for validation comparison.
   * Returns normalized values matching the validation schema.
   * @returns {Promise<object>}
   */
  async fetchApiSnapshot() {
    if (!this._hasCloudCredentials()) throw new Error('No cloud credentials');
    await this._cloud.ensureToken(this._email, this._password);
    const data = await this._cloud.getRealData(this._stationId);
    return {
      pvPowerW:         data.pvPower || 0,
      batteryPowerW:    data.batteryPower || 0,
      socPct:           data.batterySoc || 0,
      gridPowerW:       data.gridPower || 0,
      loadPowerW:       data.loadPower || 0,
      chargePowerW:     Math.max(data.batteryPower || 0, 0),
      dischargePowerW:  Math.max(-(data.batteryPower || 0), 0),
      dailyEnergyKWh:   data.dailyEnergy || 0,
      totalEnergyKWh:   data.totalEnergy || 0,
      mode:             data.batteryMode || 0,
    };
  }

  /**
   * Fetch a Modbus candidate register snapshot for validation.
   * @param {object} validator — ModbusValidator instance
   * @returns {Promise<object>} { raw: {}, derived: {} }
   */
  async fetchModbusSnapshot(validator) {
    if (!this._modbus) throw new Error('No Modbus client');
    return validator.readCandidateRegisters(this._modbus);
  }

  /**
   * Collect a simultaneous API + Modbus validation snapshot.
   * @param {object} validator — ModbusValidator instance
   * @returns {Promise<object|null>}
   */
  async collectValidationSnapshot(validator) {
    return validator.collectSnapshot(
      () => this.fetchApiSnapshot(),
      () => this.fetchModbusSnapshot(validator)
    );
  }
  ...
  async getMonthlyEnergy() {
    if (!this._hasCloudCredentials()) return 0;
    await this._cloud.ensureToken(this._email, this._password);
  ...
  async getEpsProfit() {
    if (!this._hasCloudCredentials()) return { profitToday: 0, profitTotal: 0, co2Reduction: 0 };
    await this._cloud.ensureToken(this._email, this._password);
    return this._cloud.getEpsProfit(this._stationId);
  }

  /**
   * Check if cloud credentials are available (email + password + stationId).
   * Unlike _cloudFallbackEnabled, this only checks credentials, not mode.
   */
  _hasCloudCredentials() {
    return !!(this._email && this._password && this._stationId);
  }

  /**
   * Top up Modbus data with cloud telemetry for unconfirmed fields.
   * Used in hybrid mode when Modbus has connection but no ESS/energy mapping.
   */
  async _topUpModbusFromCloud(modbusData) {
    try {
      await this._cloud.ensureToken(this._email, this._password);
      const realData = await this._cloud.getRealData(this._stationId);
      const c = modbusData.confidence;

      // Top up ESS fields if Modbus has no mapping
      if (c.batteryPower === 'none') {
        modbusData.batteryPower = realData.batteryPower || 0;
        modbusData.batterySoc   = realData.batterySoc || 0;
        modbusData.gridPower    = realData.gridPower || 0;
        modbusData.loadPower    = realData.loadPower || 0;
        modbusData.batteryMode  = realData.batteryMode || '0';
        c.batteryPower = 'confirmed';
        c.batterySoc   = 'confirmed';
        c.gridPower    = 'confirmed';
        c.loadPower    = 'confirmed';
        c.batteryMode  = 'confirmed';
      }

      // Top up energy fields if Modbus has no mapping
      if (c.totalEnergy === 'none') {
        let dailyEnergy = realData.dailyEnergy || 0;
        let totalEnergy = realData.totalEnergy || 0;
        if (!dailyEnergy && !totalEnergy) {
          const energyData = await this._cloud.getEnergyData(this._stationId);
          dailyEnergy = energyData.dailyEnergy || 0;
          totalEnergy = energyData.totalEnergy || 0;
        }
        modbusData.dailyEnergy = dailyEnergy;
        modbusData.totalEnergy = totalEnergy;
        c.dailyEnergy = 'confirmed';
        c.totalEnergy = 'confirmed';
      }

      modbusData.source = 'local+cloud';
      this.log('[Hybrid] Modbus topped up from cloud: bat=' + modbusData.batteryPower + ' soc=' + modbusData.batterySoc + ' grid=' + modbusData.gridPower + ' daily=' + modbusData.dailyEnergy + ' total=' + modbusData.totalEnergy);
    } catch (err) {
      this.log('[Hybrid] Cloud top-up for Modbus failed: ' + err.message);
    }
  }
