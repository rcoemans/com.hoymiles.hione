...
class HoymilesApi {
  ...
  async _authenticateLegacyV0() {
    const headers  = this._jsonHeaders('home');
  ...
  _buildAuthAttempts(mode) {
    ...
    // auto: try home first (most end-users), then installer, then web (no version), then legacy
    return [
  ...
  async _buildArgon2idCandidates(saltParam) {
    ...
    // Parse salt and optional Argon2id parameters
    let saltBytes;
    let m = 16384, t = 2, p = 1, hashLength = 32;

    const saltStr = String(saltParam);
    if (saltStr.startsWith('$argon2id$')) {
      // PHC format: $argon2id$v=19$m=MEM,t=TIME,p=PAR$SALT_B64
      const parts = saltStr.split('$');
      if (parts.length >= 5) {
        const paramStr = parts[3]; // e.g. "m=16384,t=2,p=1"
        const paramMap = {};
        paramStr.split(',').forEach(pair => {
          const [k, v] = pair.split('=');
          if (k && v) paramMap[k] = parseInt(v, 10);
        });
        m = paramMap.m || m;
        t = paramMap.t || t;
        p = paramMap.p || p;
        saltBytes = new Uint8Array(Buffer.from(parts[4], 'base64'));
      } else {
        saltBytes = new Uint8Array(Buffer.from(saltStr, 'base64'));
      }
    } else {
      saltBytes = new Uint8Array(Buffer.from(saltStr, 'base64'));
    }
    ...
    return [
      { variant: 'argon2id_b64',     ch: hashB64 },
      { variant: 'argon2id_hex',     ch: hashHex },
      { variant: 'argon2id_md5_b64', ch: `${md5hex}.${hashB64}` },
    ];
  }
  ...
  async _request(method, endpoint, body = {}, authenticated = true) {
    ...
    const json = res.data;
    if (!json) throw new Error(`Invalid JSON from ${endpoint}`);
