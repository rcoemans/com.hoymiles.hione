...
class HoymilesApi {
  ...
  async getRealData(stationId) {
    ...
    this.log('[HoymilesApi] Real-data sample: ' + JSON.stringify(d).substring(0, 600));

    // Some API versions nest data inside a sub-object
    const r = d.reflux_station_data || d.station_data || d.real_data || d;

    return {
      pvPower:      this._num(r.pv_power ?? r.real_power ?? r.pvPower ?? r.capacitor_power ?? r.cap_power ?? r.pv_total_power ?? 0),
      batteryPower: this._num(r.battery_power ?? r.es_power ?? r.batteryPower ?? r.storage_power ?? r.bat_power ?? 0),
      batterySoc:   this._num(r.battery_soc ?? r.es_soc ?? r.batterySoc ?? r.soc ?? 0),
      gridPower:    this._num(r.grid_power ?? r.meter_power ?? r.gridPower ?? r.reflux_power ?? 0),
      loadPower:    this._num(r.load_power ?? r.home_load_power ?? r.loadPower ?? r.home_power ?? 0),
      batteryMode:  String(r.work_mode ?? r.workMode ?? r.battery_mode ?? d.work_mode ?? d.workMode ?? 0),
    };
  }
  ...
  async getEnergyData(stationId) {
    try {
      ...
      return {
        dailyEnergy: this._num(d?.today_eq ?? d?.co_energy ?? d?.dailyEnergy ?? d?.day_eq ?? 0),
        totalEnergy: this._num(d?.total_eq ?? d?.totalEnergy ?? d?.all_eq ?? d?.cumulative_eq ?? 0),
      };
    } catch (_) {
