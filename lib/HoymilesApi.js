...
// Fault code → human-readable explanation mapping.
// Sources: Hoymiles HiOne/HiBox manual (reverse-engineered), community reports,
// Home Assistant hoymiles-cloud integration fault code lists.
// Codes are hex values reported by the cloud API in alarm_code / fault_code fields.
const FAULT_CODES = {
  // Grid / AC faults
  0x0001: 'Grid over-voltage',
  0x0002: 'Grid under-voltage',
  0x0004: 'Grid over-frequency',
  0x0008: 'Grid under-frequency',
  0x0010: 'Grid loss / no grid',
  0x0020: 'Grid impedance too high',
  0x0040: 'Grid phase imbalance',
  0x0080: 'Grid soft-start timeout',

  // Inverter / PV faults
  0x0100: 'PV over-voltage',
  0x0200: 'PV input short circuit',
  0x0400: 'Inverter over-temperature',
  0x0800: 'Inverter IGBT fault',
  0x1000: 'Inverter output over-current',
  0x2000: 'DC bus over-voltage',
  0x4000: 'DC bus under-voltage',
  0x8000: 'Fan fault',

  // Battery / BMS faults
  0x00010000: 'Battery over-voltage',
  0x00020000: 'Battery under-voltage',
  0x00040000: 'Battery over-current (charge)',
  0x00080000: 'Battery over-current (discharge)',
  0x00100000: 'Battery over-temperature',
  0x00200000: 'Battery under-temperature',
  0x00400000: 'Battery cell imbalance',
  0x00800000: 'Battery communication fault (BMS)',
  0x01000000: 'Battery state-of-health too low',
  0x02000000: 'Battery short circuit',
  0x04000000: 'Battery relay fault',
  0x08000000: 'Battery pre-charge timeout',

  // System / communication faults
  0x10000000: 'Gateway communication timeout',
  0x20000000: 'Cloud connection lost',
  0x40000000: 'Firmware update failed',
  0x80000000: 'Internal system fault',

  // String fault codes (returned as numeric strings by some API versions)
  101: 'Grid over-voltage',
  102: 'Grid under-voltage',
  103: 'Grid over-frequency',
  104: 'Grid under-frequency',
  105: 'Grid loss / no grid',
  201: 'PV over-voltage',
  202: 'PV input short circuit',
  203: 'Inverter over-temperature',
  301: 'Battery over-voltage',
  302: 'Battery under-voltage',
  303: 'Battery over-current (charge)',
  304: 'Battery over-current (discharge)',
  305: 'Battery over-temperature',
  306: 'Battery under-temperature',
  307: 'Battery cell imbalance',
  308: 'Battery communication fault (BMS)',
  401: 'Gateway communication timeout',
  402: 'Cloud connection lost',
};

/**
 * Translate a fault code value to a human-readable explanation.
 * Supports hex bitmasks (multiple faults OR'd together) and plain numeric codes.
 * Returns an empty string when no fault is active.
 */
function getFaultExplanation(code) {
  if (code === null || code === undefined || code === '' || code === 0 || code === '0') return '';

  const n = typeof code === 'string' ? (code.startsWith('0x') ? parseInt(code, 16) : parseInt(code, 10)) : Number(code);
  if (isNaN(n) || n === 0) return '';

  // Try direct lookup first (plain numeric code)
  if (FAULT_CODES[n]) return FAULT_CODES[n];

  // Try bitmask: collect all matching bit flags
  const matches = [];
  for (const [key, label] of Object.entries(FAULT_CODES)) {
    const k = Number(key);
    if (k > 0 && (n & k) === k) matches.push(label);
  }
  if (matches.length > 0) return matches.join('; ');

  // Unknown code — return it verbatim so the user knows something is wrong
  return `Unknown fault (code: ${code})`;
}
