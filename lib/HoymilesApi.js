...
class HoymilesApi {
  ...
  async getRealData(stationId) {
    ...
    // Energy data is available in the same real-data response (kWh, string)
    const dailyEnergy = this._num(d.today_eq ?? r.today_eq ?? deepFind(d, 'today_eq', 'co_energy') ?? 0);
    const totalEnergy = this._num(d.total_eq ?? r.total_eq ?? deepFind(d, 'total_eq') ?? 0);
  ...
  async getEnergyData(stationId) {
    try {
      ...
      const dailyEnergy = this._num(d?.today_eq ?? d?.co_energy ?? d?.day_eq ?? 0);
      const totalEnergy = this._num(d?.total_eq ?? d?.all_eq ?? d?.cumulative_eq ?? 0);
