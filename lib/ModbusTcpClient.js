...
class ModbusTcpClient {
  ...
  async getData() {
    ...
    try {
      ...
      // Strict energy validation: reject implausible values even from plant aggregate
      const dailyOk = plant.dailyEnergy >= 0 && plant.dailyEnergy < 200;
      const totalOk = plant.totalEnergy >= 0 && plant.totalEnergy < 100000;
      const consistent = plant.dailyEnergy <= plant.totalEnergy + 0.1 || plant.totalEnergy === 0;
      if (dailyOk && totalOk && consistent) {
        dailyEnergy = plant.dailyEnergy;
        totalEnergy = plant.totalEnergy;
        energyConfidence = CONFIDENCE.CONFIRMED;
      } else {
        this.log(`[Modbus] Plant energy REJECTED: daily=${plant.dailyEnergy}kWh total=${plant.totalEnergy}kWh (dailyOk=${dailyOk} totalOk=${totalOk} consistent=${consistent})`);
      }
  ...
  /**
   * Generate a comprehensive, shareable Modbus scan report.
   * Combines known blocks, ESS probing, and deep scan results.
   * The output is a structured JSON object suitable for analysis and sharing.
   * @returns {Promise<object>}
   */
  async exportScanReport() {
    const report = {
      timestamp: new Date().toISOString(),
      device: { host: this.host, port: this.port, unitId: this.unitId },
      knownBlocks: {},
      essProbe: null,
      summary: { readable: 0, nonZero: 0, essFound: false },
    };

    // Scan known blocks
    try {
      const blocks = await this.scanKnownBlocks();
      report.knownBlocks = blocks;
      for (const [, block] of Object.entries(blocks)) {
        if (block.readable && block.regs) {
          report.summary.readable += block.total || 0;
          report.summary.nonZero += block.regs.length;
        }
      }
    } catch (err) {
      this.log(`[Modbus] exportScanReport: known blocks failed: ${err.message}`);
    }

    // ESS probe
    try {
      const ess = await this.probeEssRegisters();
      report.essProbe = ess;
      if (ess && ess.found) {
        report.summary.essFound = true;
      }
    } catch (err) {
      this.log(`[Modbus] exportScanReport: ESS probe failed: ${err.message}`);
    }

    this.log(`[Modbus] Export scan report: ${report.summary.nonZero} non-zero in ${report.summary.readable} readable registers, ESS found: ${report.summary.essFound}`);
    return report;
  }
  ...
  _tryParseEssBlock(regs, block) {
    ...
    for (const tryPattern of patterns) {
      try {
        ...
        if (mode >= 0 && mode <= 8) plausibleCount++;
   ...
   * Uses FC06 (Write Single Register) at register 0x2001 (community-discovered).
   * @param {number} limitPct 2-100
   */
  async setPowerLimit(limitPct) {
    const pct = Math.max(2, Math.min(100, Math.round(Number(limitPct))));
    // Register value: percentage * 10 (e.g. 50% → 500)
    const value = pct * 10;
    await this._writeSingleRegister(0x2001, value);
    this.log(`[Modbus] Power limit set to ${pct}% (register value: ${value})`);
  }

  /**
   * Turn inverter on or off.
   * Uses FC06 at register 0x2000 (community-discovered).
   * @param {boolean} on true=on, false=off
   */
  async setInverterState(on) {
    const value = on ? 1 : 0;
    await this._writeSingleRegister(0x2000, value);
    this.log(`[Modbus] Inverter state set to ${on ? 'ON' : 'OFF'} (register value: ${value})`);
  }

  /**
   * Write a single 16-bit register (FC06).
   * @param {number} register
   * @param {number} value
   */
  async _writeSingleRegister(register, value) {
    const txId = nextTxId();
    const buf = Buffer.alloc(12);
    buf.writeUInt16BE(txId, 0);          // Transaction ID
    buf.writeUInt16BE(0, 2);             // Protocol ID
    buf.writeUInt16BE(6, 4);             // Length
    buf.writeUInt8(this.unitId, 6);      // Unit ID
    buf.writeUInt8(0x06, 7);             // FC06: Write Single Register
    buf.writeUInt16BE(register, 8);      // Register address
    buf.writeUInt16BE(value, 10);        // Register value

    const response = await sendReceive(this.host, this.port, buf);
    if (!response || response.length < 12) throw new Error('Modbus write response too short');
    const fc = response.readUInt8(7);
    if (fc & 0x80) {
      const exCode = response.length > 8 ? response.readUInt8(8) : 0;
      throw new Error(`Modbus write exception ${exCode}`);
    }
  }
