...
class HioneMapper {
   ...
   * S-Miles Cloud API returns kWh (e.g. today_eq: "5", total_eq: "125").
   * Local API already returns kWh via its _kwh() helper.
   * Safety net: if value > 100000 assume Wh and convert (edge case only).
   */
  static _normKwh(v) {
    let n = HioneMapper._num(v);
    if (Math.abs(n) > 100000) n = Math.round(n / 10) / 100; // Wh → kWh
    return n;
  }
}
