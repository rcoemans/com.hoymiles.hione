...
function normalizeRealData(raw) {
  ...
  // Some API versions return SoC as 0–1 decimal instead of 0–100 percentage
  const rawSoc = raw.batterySoc;
  const normalizedSoc = (rawSoc != null && rawSoc <= 1 && rawSoc >= 0) ? rawSoc * 100 : rawSoc;
  const batterySoc = validate(normalizedSoc, { min: 0, max: 100 });
