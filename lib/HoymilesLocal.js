...
// ─── Command type bytes ────────────────────────────────────────────────────────
// Command bytes from hoymiles-wifi (suaveolent/hoymiles-wifi)
// App → DTU commands (0xa3xx), Gateway (0xdbxx), Energy Storage (0xc3xx)
const CMD = {
  REAL_DATA:                 [0xa3, 0x03],   // CMD_REAL_DATA_RES_DTO
  REAL_DATA_NEW:             [0xa3, 0x11],   // CMD_REAL_RES_DTO
  GATEWAY_INFO:              [0xdb, 0x01],   // CMD_GW_INFO_RES_DTO
  GATEWAY_NETWORK_INFO:      [0xdb, 0x06],   // CMD_GW_NET_INFO_RES
  ENERGY_STORAGE_REGISTRY:   [0xc3, 0x02],   // CMD_ES_REG_RES_DTO
  ENERGY_STORAGE_DATA:       [0xc3, 0x03],   // CMD_ES_DATA_DTO
  SET_ENERGY_STORAGE_MODE:   [0xc3, 0x08],   // CMD_ES_USER_SET_RES_DTO
  SET_POWER_LIMIT:           [0xa3, 0x05],   // CMD_ACTIVE_POWER_LIMIT
  SET_INVERTER_STATE:        [0xa3, 0x06],   // CMD_INVERTER_ON_OFF
};
...
function extractPayload(frame) {
  ...
  // Read the length field at bytes 8-9 (total frame length incl. header)
  const readLength = (frame[8] << 8) | frame[9];
  if (readLength > 10 && readLength <= frame.length) {
    return frame.slice(10, readLength);
  }
...
// Protobuf field layout matches hoymiles-wifi RealDataResDTO:
//   field 1: time_ymd_hms (bytes)  — e.g. "2025-06-14 22:00:00"
//   field 3: error_code (varint)   — 0
//   field 4: offset (varint)       — 28800
//   field 5: time (varint)         — unix timestamp
function makeRealDataReq(offset = 28800) {
  const now = new Date();
  ...
  return Buffer.concat([
    PB.bytes(1, Buffer.from(timeStr, 'utf8')),
    PB.varint(3, 0),
    PB.varint(4, offset),
...
function makePowerLimitReq(limitPct, offset = 28800) {
  return Buffer.concat([
    PB.varint(1, offset),
    PB.varint(2, Math.floor(Date.now() / 1000)),
    PB.varint(3, Math.max(2, Math.min(100, Math.round(limitPct)))),
  ]);
}

function makeInverterStateReq(on, offset = 28800) {
  return Buffer.concat([
    PB.varint(1, offset),
    PB.varint(2, Math.floor(Date.now() / 1000)),
    PB.varint(3, on ? 1 : 0),
...
function sendReceive(host, port, frame) {
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', chunk => {
      ...
      // Once we have the 10-byte header, check if we have the full frame
      if (totalLen >= 10) {
        const combined = Buffer.concat(chunks);
        const expectedLen = (combined[8] << 8) | combined[9];
        if (totalLen >= expectedLen && expectedLen >= 10) {
          done(null, combined);
        }
      }
    });
...
class HoymilesLocal {
  ...
  async getRealData() {
    // Try RealData (0xa303) first — most widely supported
    const payload  = makeRealDataReq();
    const frame    = buildFrame(CMD.REAL_DATA, payload, this._nextSeq());
    const response = await sendReceive(this.host, this.port, frame);
    const raw      = extractPayload(response);
    const f        = PB.parse(raw);

    this.log(`[Local] getRealData → fields: ${Object.keys(f).join(',')}`);

    // The RealDataReqDTO response has nested PvDataMO at field 10 and
    // MeterDataMO at field 8 — both are length-delimited (wire type 2).
    // Parse nested sub-messages if present.
    let pvPower = 0, gridPower = 0;
    if (Buffer.isBuffer(f[10])) {
      const pv = PB.parse(f[10]);
      pvPower = this._deciWatt(pv[5] ?? 0);  // pv_power (field 5)
    }
    if (Buffer.isBuffer(f[8])) {
      const meter = PB.parse(f[8]);
      gridPower = this._deciWatt(meter[3] ?? 0);  // meter_total_power (field 3)
    }

    return {
      pvPower,
      batteryPower: 0,  // Not in basic RealData — enriched from ES data
      batterySoc:   0,
      gridPower,
      loadPower:    0,
      batteryMode:  '0',
  ...
  /**
   * Set inverter output power limit (2-100%).
   * WARNING: writes to EEPROM — max 10 changes/day recommended.
   */
  async setPowerLimit(limitPct) {
    const pct      = Math.max(2, Math.min(100, Math.round(limitPct)));
    const payload  = makePowerLimitReq(pct);
    const frame    = buildFrame(CMD.SET_POWER_LIMIT, payload, this._nextSeq());
    const response = await sendReceive(this.host, this.port, frame);
    const f        = PB.parse(extractPayload(response));
    const status   = f[1] ?? 0;
    if (status !== 0) throw new Error(`Gateway rejected power limit change (status ${status})`);
    this.log(`[Local] setPowerLimit(${pct}%) → OK`);
    return true;
  }
  ...
  async setInverterState(on) {
    const payload  = makeInverterStateReq(on);
    const frame    = buildFrame(CMD.SET_INVERTER_STATE, payload, this._nextSeq());
    const response = await sendReceive(this.host, this.port, frame);
    const f        = PB.parse(extractPayload(response));
    const status   = f[1] ?? 0;
    if (status !== 0) throw new Error(`Gateway rejected inverter state change (status ${status})`);
    this.log(`[Local] setInverterState(${on ? 'ON' : 'OFF'}) → OK`);
    return true;
  }
  ...
  _deciWatt(raw) {
    // Hoymiles protobuf often sends power in 0.1W units
    const v = Number(raw);
    if (isNaN(v)) return 0;
    // Values >100000 are in 0.1W; otherwise assume watts
    return v > 100_000 ? Math.round(v / 10) : v;
  }
